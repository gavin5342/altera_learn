...
module top_altera_merlin_axi_slave_ni_19129_jimy6za
#(
    // -----------------------------------------------
    ...
    parameter PKT_DATALESS                = 130,
    parameter PKT_EOP_OOO                 = 130,
    parameter PKT_SOP_OOO                 = 131,

    // AXI5LITE new packet fields
    parameter PKT_AWAKEUP                 = 132,
    parameter PKT_ATRACE                  = 133,
    parameter PKT_TRACE                   = 134,   
    parameter PKT_POISON_L                = 135,
    parameter PKT_POISON_H                = 136,
    parameter PKT_DATACHK_L               = 137,
    parameter PKT_DATACHK_H               = 138,
    parameter PKT_ADDRCHK_L               = 139,
    parameter PKT_ADDRCHK_H               = 140,
    parameter PKT_SAI_L                   = 141,
    parameter PKT_SAI_H                   = 142,    
    parameter PKT_USER_DATA_L             = 143,
    parameter PKT_USER_DATA_H             = 144,
    
    // ACE5Lite new packet fields
    ...
    parameter SKIP_ADDR_ALIGNMENT      = 0,
    ...
    parameter ENABLE_SPLIT_AT_4K          = 0,    // use to add or not 4k boundary splitter
    // -----------------------------------------------
    // Derived parameters 
    // -----------------------------------------------
...
(
    ...
    output           [DATACHK_WIDTH-1:0]          wdatachk,
    output           [POISON_WIDTH-1:0]           wpoison,
    output                                        wtrace,    
    ...
    input            [1:0]                        bresp,
    input            [RESP_USER_WIDTH-1:0]        buser,
    input                                         bvalid,
    input                                         btrace,
    output                                        bready,
    ...
    input            [DATACHK_WIDTH-1:0]      rdatachk,
    input            [POISON_WIDTH-1:0]       rpoison,
    input                                     rtrace,    
    input                                     rlast,
    input                                     rvalid,
    output                                    rready, 
    ...
    output [1:0]                              awdomain, 
    output [AWSNOOP_WIDTH-1:0]                awsnoop, 
    output [1:0]                              awbar, 
    output                                    awunique, 
    
    // ACE5 Lite
    output [5:0]                          awatop,
    output [10:0]                         awstashnid,  
    output                                awstashniden,      
    output [4:0]                          awstashlpid, 
    output                                awstashlpiden, 
    output                                awmmusecsid,
    output [SID_WIDTH-1:0]                awmmusid,
    output                                armmusecsid,
    output [SID_WIDTH-1:0]                armmusid,
    
    output [ADDRCHK_WIDTH-1:0]              awaddrchk,
    output [ADDRCHK_WIDTH-1:0]              araddrchk,
    ...
    localparam  PKT_DATACHK_W       = PKT_DATACHK_H - PKT_DATACHK_L + 1;
    localparam META_DATA_W          = ST_DATA_W - AXI_SLAVE_ID_W ;
        localparam ST_PIPELINE_DATA_W   =  POISON_WIDTH + DATACHK_WIDTH + USER_DATA_WIDTH + DATA_USER_WIDTH + AXI_SLAVE_ID_W + RESPONSE_W + PKT_DATA_W + 1; // added user_data
        localparam ST_PIPELINE_WR_DATA_W   = DATA_USER_WIDTH + AXI_SLAVE_ID_W + 2; 
    // --------------------------------------------------
    // Ceil(log2()) function
    // --------------------------------------------------
    ...
    reg  [BYTECOUNT_W-1:0]    write_cmd_bytecount_for_cmd;
    ...
    wire [DATACHK_WIDTH-1:0]  write_cmd_wdatachk;
    wire                      write_cmd_wtrace;
    wire [POISON_WIDTH-1:0]   write_cmd_wpoison;
    wire                      write_cmd_awakeup;
    wire                      write_cmd_awtrace;        
    wire                      write_cmd_artrace; 
    
    wire [5:0]                write_cmd_awatop;
    wire [10:0]               write_cmd_awstashnid;
    wire                      write_cmd_awstashniden;
    wire [4:0]                write_cmd_awstashlpid;
    wire                      write_cmd_awstashlpiden;
    wire                      write_cmd_awmmusecsid;
    wire [SID_WIDTH-1:0]      write_cmd_awmmusid;
    wire [ADDRCHK_WIDTH-1:0]  write_cmd_awaddrchk;
    ...
    wire read_cmd_artrace;    
    wire                      read_cmd_armmusecsid;
    wire [SID_WIDTH-1:0]      read_cmd_armmusid;
    wire [ADDRCHK_WIDTH-1:0]  read_cmd_araddrchk;

    wire [3:0]                read_cmd_snoop;
    wire [1:0]                read_cmd_domain;
    wire [1:0]                read_cmd_bar;

    wire [AWSNOOP_WIDTH-1:0]  write_cmd_snoop;
    wire [1:0]                write_cmd_domain;
    wire [1:0]                write_cmd_bar;
    wire                      write_cmd_unique;
    wire                      write_cmd_data_less;
    ...
    wire [META_DATA_W + AXI_SLAVE_ID_W : 0]     read_rsp_mem_indata;
    wire [META_DATA_W + AXI_SLAVE_ID_W : 0]     read_rsp_mem_outdata;

    wire [META_DATA_W -1 :0]                    meta_write_data_resp_mem_in;
    wire [META_DATA_W + AXI_SLAVE_ID_W : 0]     write_rsp_mem_indata;
    wire [META_DATA_W + AXI_SLAVE_ID_W : 0]     write_rsp_mem_outdata;
    ...
    wire p1_btrace;
    wire [ST_PIPELINE_WR_DATA_W-1 : 0] p1_bdata_in,p1_bdata_out;
    
    wire [PKT_DATA_W-1:0]p1_rdata;
    wire [RESPONSE_W-1:0] p1_rresp,p1_bresp;
    wire [DATA_USER_WIDTH-1:0]p1_ruser;
    wire [RESP_USER_WIDTH-1:0]p1_buser;
    wire [AXI_SLAVE_ID_W-1:0]p1_rid,p1_bid;
    wire [AXI_SLAVE_ID_W-1:0]rid_resp_mem,bid_resp_mem;    
    wire p1_rlast,p1_rtrace;
    wire [DATACHK_WIDTH-1:0] p1_rdatachk;
    wire [POISON_WIDTH-1:0] p1_rpoison;
    wire [USER_DATA_WIDTH-1:0] p1_ruser_data;
    ...
    logic single_read_data_exp,ignore_bvalid;
   ...
   endgenerate
    ...
    //------------------------------------
    // Instantiate 4K splitter before other logic
    //------------------------------------
    // Av-st read sink command packet interface
    wire                         rd_cp_ready;
    wire                         rd_cp_valid;
    wire [ST_DATA_W-1:0]         rd_cp_data;
    wire [ST_CHANNEL_W-1:0]      rd_cp_channel;
    wire                         rd_cp_startofpacket;
    wire                         rd_cp_endofpacket;
    wire                         wr_cp_ready;
    wire                         wr_cp_valid;
    localparam  ST_DATA_OUT_W   = ST_DATA_W + (PKT_BYTE_CNT_H-PKT_BYTE_CNT_L+1)+1 ; // 1 bit indicating 4k split    
    wire [ST_DATA_W:0]           wr_cp_data; // MSB tells you 4K cross
    wire [ST_CHANNEL_W-1:0]      wr_cp_channel;
    wire                         wr_cp_startofpacket;
    wire                         wr_cp_endofpacket;

generate 
//Handle Write Commands
    if (ENABLE_SPLIT_AT_4K) begin : SPLIT_AT_4K
        wr_burst_split_4k # (
        ...
        .PADDING_ADDR_MERLIN  (PADDING_ADDR_MERLIN),
        ...
        .aresetn                (aresetn),
        // sink side
        .sink_cp_ready          (write_cp_ready        ),
        .sink_cp_valid          (write_cp_valid        ),
        .sink_cp_data           (write_cp_data         ),
        .sink_cp_channel        (write_cp_channel      ),
        .sink_cp_startofpacket  (write_cp_startofpacket),
        .sink_cp_endofpacket    (write_cp_endofpacket  ),
        // source side
        .source_cp_ready        (wr_cp_ready        ),
        .source_cp_valid        (wr_cp_valid        ),
        .source_cp_data         (wr_cp_data         ),
        .source_cp_channel      (wr_cp_channel      ),
        .source_cp_startofpacket(wr_cp_startofpacket),
        .source_cp_endofpacket  (wr_cp_endofpacket  )
        );
//Handle Read Commands
        ...
        .aresetn                (aresetn),
        // sink side
        .sink_cp_ready          (read_cp_ready        ),
        .sink_cp_valid          (read_cp_valid        ),
        .sink_cp_data           (read_cp_data         ),
        .sink_cp_channel        (read_cp_channel      ),
        .sink_cp_startofpacket  (read_cp_startofpacket),
        .sink_cp_endofpacket    (read_cp_endofpacket  ),
        // source side
        .source_cp_ready        (rd_cp_ready        ),
        .source_cp_valid        (rd_cp_valid        ),
        .source_cp_data         (rd_cp_data         ),
        .source_cp_channel      (rd_cp_channel      ),
        .source_cp_startofpacket(rd_cp_startofpacket),
        .source_cp_endofpacket  (rd_cp_endofpacket  )
        );
    end
    else begin : NO_SPLITTING
        assign read_cp_ready      = rd_cp_ready           ;
        assign rd_cp_valid        = read_cp_valid         ;
        assign rd_cp_data         = read_cp_data          ;
        assign rd_cp_channel      = read_cp_channel       ;
        assign rd_cp_startofpacket= read_cp_startofpacket ;
        assign rd_cp_endofpacket  = read_cp_endofpacket   ;

        assign write_cp_ready     = wr_cp_ready            ;
        assign wr_cp_valid        = write_cp_valid         ;
        assign wr_cp_data         = write_cp_data          ;
        assign wr_cp_channel      = write_cp_channel       ;
        assign wr_cp_startofpacket= write_cp_startofpacket ;
        assign wr_cp_endofpacket  = write_cp_endofpacket   ;
    end
endgenerate
    //------------------------------------
    ...
    assign write_cmd_data        = wr_cp_data[PKT_DATA_H : PKT_DATA_L];
    assign write_cmd_byteen      = wr_cp_data[PKT_BYTEEN_H : PKT_BYTEEN_L];
    assign write_cmd_mid         = wr_cp_data[PKT_SRC_ID_H : PKT_SRC_ID_L];
    assign write_cmd_size        = wr_cp_data[PKT_BURST_SIZE_H : PKT_BURST_SIZE_L];
    assign write_cmd_lock        = { 1'b0, wr_cp_data[PKT_TRANS_EXCLUSIVE] };
    generate
      if (PKT_ADDR_H > PKT_ADDR_L + ADDR_WIDTH -1) begin
           assign write_cmd_addr   = wr_cp_data[PKT_ADDR_L + ADDR_WIDTH -1 : PKT_ADDR_L];
      end
      else if (PKT_ADDR_H < PKT_ADDR_L + ADDR_WIDTH -1) begin
           assign write_cmd_addr   = {{PADDING_ADDR_MERLIN{1'b0}},wr_cp_data[PKT_ADDR_H : PKT_ADDR_L]};
      end
      else begin 
           assign write_cmd_addr   = wr_cp_data[PKT_ADDR_H : PKT_ADDR_L];
      end
    endgenerate
    assign write_cmd_bytecount_for_cmd  = wr_cp_data[PKT_BYTE_CNT_H : PKT_BYTE_CNT_L];// after 4k split  original 
    assign write_cmd_bytecount          = wr_cp_data[PKT_BYTE_CNT_H : PKT_BYTE_CNT_L];// after 4k split  original 
    //assign write_cmd_bytecount   = wr_cp_data[ST_DATA_OUT_W-1] ? wr_cp_data[ST_DATA_OUT_W-2:ST_DATA_OUT_W-2-(BYTECOUNT_W)] :
    //                                                             wr_cp_data[PKT_BYTE_CNT_H : PKT_BYTE_CNT_L];         
    assign write_cmd_threadid    = wr_cp_data[PKT_THREAD_ID_H : PKT_THREAD_ID_L];
    assign write_cmd_protection  = wr_cp_data[PKT_PROTECTION_H : PKT_PROTECTION_L];
    assign write_cmd_bursttype   = wr_cp_data[PKT_BURST_TYPE_H : PKT_BURST_TYPE_L];
    assign write_cmd_user        = wr_cp_data[PKT_ADDR_SIDEBAND_H : PKT_ADDR_SIDEBAND_L];
    assign write_cmd_qos         = wr_cp_data[PKT_QOS_H : PKT_QOS_L];
    assign awlen_wire            = (write_cmd_bytecount_for_cmd >> log2ceil(PKT_DATA_W / 8)) - 1;
    ...
    generate
       if (ROLE_BASED_USER) begin
           ...
           if(POISON_WIDTH == PKT_POISON_W)
               assign write_cmd_wuser_poison  = wr_cp_data[PKT_POISON_H : PKT_POISON_L ];
           else
               assign write_cmd_wuser_poison  = {{(POISON_WIDTH - PKT_POISON_W){1'b0}} , wr_cp_data[PKT_POISON_H : PKT_POISON_L ]};
               
           if(SAI_WIDTH == PKT_SAI_W)
               assign write_cmd_awuser_sai    = wr_cp_data [PKT_SAI_H : PKT_SAI_L];
           else
               assign write_cmd_awuser_sai    = {{(SAI_WIDTH - PKT_SAI_W){1'b0}}, wr_cp_data[PKT_SAI_H:PKT_SAI_L ]};
               
           assign aw_parity_status = wr_cp_data[PKT_ADDRCHK_H];
       
           if(USE_PKT_ADDRCHK)
               assign write_cmd_awuser_addrchk = wr_cp_data[PKT_ADDRCHK_H-1 :PKT_ADDRCHK_L];
           else begin
           ...
           if(USE_PKT_DATACHK)
               assign write_cmd_wuser_datachk = wr_cp_data[PKT_DATACHK_H : PKT_DATACHK_L]; 
           else
               assign write_cmd_wuser_datachk = '0;
           
           assign write_cmd_wuser_data = wr_cp_data[PKT_USER_DATA_H : PKT_USER_DATA_L]; 
    ...
    // pass through ace_lite signals
       assign write_cmd_domain         = wr_cp_data[PKT_DOMAIN_H : PKT_DOMAIN_L];
       if (ACE5_LITE_SUPPORT == 0) 
           assign write_cmd_snoop          = wr_cp_data[PKT_SNOOP_H-1 : PKT_SNOOP_L];   // 3 bits
       else 
           assign write_cmd_snoop          = wr_cp_data[PKT_SNOOP_H : PKT_SNOOP_L];   // 4 bits
       assign write_cmd_bar            = wr_cp_data[PKT_BARRIER_H : PKT_BARRIER_L];
       assign write_cmd_unique         = wr_cp_data[PKT_WUNIQUE];
       assign write_cmd_data_less      = wr_cp_data[PKT_DATALESS];
       assign write_cmd_cache          = wr_cp_data[PKT_CACHE_H : PKT_CACHE_L];
       
    //ACE5-Lite Signals 
    
    assign write_cmd_awatop        = wr_cp_data[PKT_AWATOP_H      : PKT_AWATOP_L] ;
    assign write_cmd_awstashnid    = wr_cp_data[PKT_AWSTASHNID_H  : PKT_AWSTASHNID_L] ;  
    assign write_cmd_awstashniden  = wr_cp_data[PKT_AWSTASHNIDEN] ;      
    assign write_cmd_awstashlpid   = wr_cp_data[PKT_AWSTASHLPID_H : PKT_AWSTASHLPID_L] ; 
    assign write_cmd_awstashlpiden = wr_cp_data[PKT_AWSTASHLPIDEN] ; 
    assign write_cmd_awmmusecsid   = wr_cp_data[PKT_MMUSECSID ] ;
    assign write_cmd_awmmusid      = wr_cp_data[PKT_MMUSID_H    : PKT_MMUSID_L] ;
    ...
    assign awatop        = write_cmd_awatop;
    assign awstashnid    = write_cmd_awstashnid;
    assign awstashniden  = write_cmd_awstashniden;
    assign awstashlpid   = write_cmd_awstashlpid;
    assign awstashlpiden = write_cmd_awstashlpiden;
    assign awmmusecsid   = write_cmd_awmmusecsid;
    assign awmmusid      = write_cmd_awmmusid;
    generate 
    ...
    endgenerate
    // might need to add AXI5 handling condition ???
     generate
         if (AXI_VERSION == "AXI4") begin
             assign write_cmd_wuser  = wr_cp_data[PKT_DATA_SIDEBAND_H : PKT_DATA_SIDEBAND_L];
             assign wuser            = write_cmd_wuser;
             assign awqos            = write_cmd_qos;
             assign awregion         = 4'b0;
         end else if  (AXI_VERSION == "AXI4Lite") begin
    ...
    generate
      if(ENABLE_OOO == 0)begin
        assign write_rsp_fifo_indata[ST_DATA_W]     = (data_less_txn) ? 1'h1 : wr_cp_endofpacket;
        assign write_rsp_fifo_indata[ST_DATA_W-1:0] = wr_cp_data[ST_DATA_W-1:0];
      end
    endgenerate  
   ...
   endgenerate 
    ...
    begin
        ...
        if (PASS_ID_TO_SLAVE) begin
           ...
           if (AXI_VERSION == "AXI4" || AXI_VERSION == "AXI4Lite") begin
    ...
    assign internal_write_cp_endofburst =  (data_less_txn) ? wr_cp_endofpacket  : (write_cmd_bytecount == minimum_bytecount);
    ...
    //------------------------------------
    assign  wr_cp_ready = (aw_w_ready && write_rsp_fifo_inready) || (address_taken && wready) || (data_taken && awready);

    assign  aw_w_ready = (data_less_txn) ? awready : (awready && wready) ;
    assign  awvalid = wr_cp_valid && !address_taken && write_rsp_fifo_inready;  
    assign  wvalid_tmp  = wr_cp_valid && !data_taken && write_rsp_fifo_inready;    
    assign  wvalid  =  (data_less_txn) ? 'h0 : wvalid_tmp ;
    ...
    if (SYNC_RESET == 0) begin : async_rst2
       ...
       always_ff @(posedge aclk or negedge aresetn) begin
           if (!aresetn) begin
               ...
               if (awvalid && awready)
                   address_taken   <= '1;
               if (wr_cp_valid && wr_cp_ready && internal_write_cp_endofburst) // address is considered taken until end of packet
                   address_taken   <= '0;
           end        
       end
    end // async_rst2
    ...
    if (SYNC_RESET == 0) begin : async_rst3
       always_ff @(posedge aclk or negedge aresetn) begin
           if  (!aresetn) begin
               ...
               if (wvalid_tmp && wready)
                   data_taken  <= '1;
               if (wr_cp_valid && wr_cp_ready)
                   data_taken  <= '0;
           end
       end
    end // async_rst3
    ...
    logic internal_write_to_fifo ;
    assign internal_write_to_fifo  = (data_less_txn) ? wr_cp_startofpacket : internal_write_cp_endofburst ;
    assign write_rsp_fifo_invalid  = wr_cp_ready && wr_cp_valid && internal_write_to_fifo;
    ...
    generate 
        if (PKT_ADDR_H > PKT_ADDR_L + ADDR_WIDTH -1) begin
             assign read_cmd_addr        = rd_cp_data[PKT_ADDR_L + ADDR_WIDTH -1 : PKT_ADDR_L];
        end
        else if (PKT_ADDR_H < PKT_ADDR_L + ADDR_WIDTH -1) begin
             assign read_cmd_addr        = {{PADDING_ADDR_MERLIN{1'b0}},rd_cp_data[PKT_ADDR_H : PKT_ADDR_L]};
        end
        else begin 
             assign read_cmd_addr        = rd_cp_data[PKT_ADDR_H : PKT_ADDR_L];
        end
    endgenerate
    assign read_cmd_mid             = rd_cp_data[PKT_SRC_ID_H :PKT_SRC_ID_L];
    assign read_cmd_bytecount       = rd_cp_data[PKT_BYTE_CNT_H :PKT_BYTE_CNT_L];    
    assign read_cmd_size            = rd_cp_data[PKT_BURST_SIZE_H:PKT_BURST_SIZE_L];
    assign read_cmd_threadid        = rd_cp_data[PKT_THREAD_ID_H:PKT_THREAD_ID_L];
    assign read_cmd_protection      = rd_cp_data[PKT_PROTECTION_H:PKT_PROTECTION_L];
    assign read_cmd_bursttype       = rd_cp_data[PKT_BURST_TYPE_H:PKT_BURST_TYPE_L];
    assign read_cmd_user            = rd_cp_data[PKT_ADDR_SIDEBAND_H : PKT_ADDR_SIDEBAND_L];
    assign read_cmd_compressed      = rd_cp_data[PKT_TRANS_COMPRESSED_READ];
    assign read_cmd_lock            = { 1'b0, rd_cp_data[PKT_TRANS_EXCLUSIVE] };
    ...
    generate
      if(ROLE_BASED_USER) begin
        ...
        assign ar_parity_status = rd_cp_data[PKT_ADDRCHK_H];
        if(USE_PKT_ADDRCHK)
            assign read_cmd_aruser_addrchk = rd_cp_data[PKT_ADDRCHK_H-1 : PKT_ADDRCHK_L];
        else begin
            if(SKIP_USER_ADDRCHK_CAL) begin
        ...
        if(SAI_WIDTH == PKT_SAI_W)
            assign read_cmd_aruser_sai           = rd_cp_data [PKT_SAI_H : PKT_SAI_L];
        else
            assign read_cmd_aruser_sai           = {{(SAI_WIDTH - PKT_SAI_W){1'b0}}, rd_cp_data[PKT_SAI_H:PKT_SAI_L ]};
      end else begin
    ...
    // its consistent with system domain.
    // pass through ace_lite signals
       assign read_cmd_domain          = rd_cp_data[PKT_DOMAIN_H : PKT_DOMAIN_L];    
       assign read_cmd_snoop           = rd_cp_data[PKT_SNOOP_H : PKT_SNOOP_L];      // 4 bits
       assign read_cmd_bar             = rd_cp_data[PKT_BARRIER_H : PKT_BARRIER_L];
       assign read_cmd_cache           = rd_cp_data[PKT_CACHE_H:PKT_CACHE_L];
       
       assign read_cmd_armmusecsid    = rd_cp_data[PKT_MMUSECSID] ;
       assign read_cmd_armmusid       = rd_cp_data[PKT_MMUSID_H : PKT_MMUSID_L] ;
    ...
    if ((AXI_VERSION == "AXI4" && ACE_LITE_SUPPORT == 1) || (AXI_VERSION == "AXI5" && ACE5_LITE_SUPPORT == 1) )
        assign single_read_data_exp = (  (((arsnoop == 'h8 || arsnoop == 'h9 ||  arsnoop == 'hd) && (ardomain == 'h0 || ardomain == 'h1 || ardomain == 'h2) ) 
                                       || ((arsnoop == 'hb || arsnoop == 'hc)                    && (ardomain == 'h1 || ardomain == 'h2)) )
                                       &&  arbar[0] == 'h0 ) ;
    else 
        assign single_read_data_exp = 'h0 ;
    assign armmusecsid  = read_cmd_armmusecsid;
    assign armmusid     = read_cmd_armmusid;
    ...
    assign data_less_txn= write_cmd_data_less;
    generate
      if (ENABLE_OOO == 0) begin
    ...
    generate
        if (AXI_VERSION == "AXI4") begin
            ...
            assign read_cmd_qos = rd_cp_data[PKT_QOS_H : PKT_QOS_L];
            assign arqos        = read_cmd_qos;
            assign arregion     = 4'b0;
        end else if (AXI_VERSION == "AXI5") begin
            assign arqos        = rd_cp_data[PKT_QOS_H : PKT_QOS_L];
            assign arregion     = '0;
        end else begin
            assign arqos        = '0;
            assign arregion     = '0;
    ...
    //Adding this specifically for HPS limitation
    //Narrow transfers are not supported
    //This tells the width adapter what was the original address
    if (SKIP_ADDR_ALIGNMENT) begin
        assign read_cmd_addr_aligned    = read_cmd_addr[ADDR_WIDTH-1:0];
    end else begin
    ...
    always_comb
        begin
            ...
            if (single_read_data_exp) 
                read_cp_data_for_fifo[PKT_BYTE_CNT_H:PKT_BYTE_CNT_L] = 1 << $clog2(NUMSYMBOLS);
            else 
                read_cp_data_for_fifo[PKT_BYTE_CNT_H:PKT_BYTE_CNT_L] =  read_cp_data[PKT_BYTE_CNT_H:PKT_BYTE_CNT_L] ;
        end
    ...
    always_comb
        begin
            ...
            if (PASS_ID_TO_SLAVE) begin
               ...
               if (ROLE_BASED_USER) begin
                  read_rp_data_temp[PKT_POISON_H:PKT_POISON_L]                    = p1_rpoison; // rpoison = 0
                  read_rp_data_temp[PKT_USER_DATA_H:PKT_USER_DATA_L]              = p1_ruser_data;
                  if(USE_PKT_DATACHK)
                      read_rp_data_temp[PKT_DATACHK_H:PKT_DATACHK_L]              = p1_rdatachk;
                  else             
                      read_rp_data_temp[PKT_DATACHK_H:PKT_DATACHK_L]              = calcParity(p1_rdata);
               end
        end 
// start of packet assertion should only happen at the start of burst and should not be asserted at the start of sub-burst
    ...
    assign rd_cp_ready    = arready && read_rsp_fifo_inready; //backpressure when the fifo is full
   generate
      assign arvalid = rd_cp_valid && read_rsp_fifo_inready; //its the job of the network to hold the valid signal, until the slave asserts the arready.      
 ...
 generate 
   if (ENABLE_OOO == 0) begin
    ...
    top_altera_merlin_axi_slave_ni_altera_avalon_sc_fifo_19129_fs67l3a #(
        .SYMBOLS_PER_BEAT    (1),
    ...
    top_altera_merlin_axi_slave_ni_altera_avalon_sc_fifo_19129_fs67l3a #(
     ...
     //N2W milestone 1-3 achive shift register logic is implemented instead of response FIFO
      assign meta_write_data_resp_mem_in  =  wr_cp_data[ST_DATA_W-1: PKT_ADDR_L];                               
      assign write_rsp_mem_indata         = {wr_cp_endofpacket,meta_write_data_resp_mem_in , awid_internal[AXI_SLAVE_ID_W -1:0]}; 
//      only way to add ID field without changing frame format is keep ID at MSB
//      - EOP, ID, meta_data-(except data and BE)
      assign write_rsp_fifo_outdata[ST_DATA_W]               = write_rsp_mem_outdata[META_DATA_W + AXI_SLAVE_ID_W] ; // EOP
      assign write_rsp_fifo_outdata[ST_DATA_W-1:PKT_ADDR_L]  = write_rsp_mem_outdata[META_DATA_W + AXI_SLAVE_ID_W-1:AXI_SLAVE_ID_W] ; // rest frame till ADDR_L
     
      wr_response_mem_jimy6za #(
       ...
       .PIPELINE_CMP (0)
          ...
          .out_data         (write_rsp_mem_outdata),                
          .out_valid        (write_rsp_fifo_outvalid),               
      ...
      assign meta_read_data_resp_mem_in  =   read_cp_data_for_fifo [ST_DATA_W-1: PKT_ADDR_L];                               
      assign read_rsp_mem_indata         =  {rd_cp_endofpacket, meta_read_data_resp_mem_in , arid_internal[AXI_SLAVE_ID_W -1:0]}; 
      //      only way to add ID field without changing frame format is keep ID at MSB
//      - EOP, ID, meta_data-(except data and BE)
      assign read_rsp_fifo_outdata[ST_DATA_W]               = read_rsp_mem_outdata[META_DATA_W + AXI_SLAVE_ID_W] ; // EOP
      assign read_rsp_fifo_outdata[ST_DATA_W-1:PKT_ADDR_L]  = read_rsp_mem_outdata[META_DATA_W + AXI_SLAVE_ID_W-1:AXI_SLAVE_ID_W] ; // rest frame till ADDR_L
     
      rd_response_mem_jimy6za #(
       .ST_DATA_W    (META_DATA_W + AXI_SLAVE_ID_W +1), // adding LSB arid info
       .ID_W         (AXI_SLAVE_ID_W),
       .PIPELINE_OUT (1),
       .PIPELINE_CMP (0)
...
generate 
   if (ENABLE_OOO == 1) begin
     assign p1_data_in  = {rlast,ruser_data,ruser_datachk,ruser_poison,ruser,rresp,rid,rdata};
     ...
     assign p1_rpoison    = p1_data_out[PKT_DATA_W+AXI_SLAVE_ID_W+RESPONSE_W+DATA_USER_WIDTH +: POISON_WIDTH];     
     assign p1_rdatachk   = p1_data_out[PKT_DATA_W+AXI_SLAVE_ID_W+RESPONSE_W+DATA_USER_WIDTH+POISON_WIDTH +: DATACHK_WIDTH];
     assign p1_ruser_data = p1_data_out[PKT_DATA_W+AXI_SLAVE_ID_W+RESPONSE_W+DATA_USER_WIDTH+POISON_WIDTH+DATACHK_WIDTH +: USER_DATA_WIDTH];
     assign p1_rlast    = p1_data_out[ST_PIPELINE_DATA_W-1];
    

    
    top_altera_merlin_axi_slave_ni_altera_avalon_st_pipeline_stage_19129_a5cmeay #(
      .SYMBOLS_PER_BEAT (1),
     ...
     assign p1_buser    = p1_bdata_out[AXI_SLAVE_ID_W+2 +: RESP_USER_WIDTH];
    top_altera_merlin_axi_slave_ni_altera_avalon_st_pipeline_stage_19129_zyff54i #(
      .SYMBOLS_PER_BEAT (1),
      .BITS_PER_SYMBOL  (ST_PIPELINE_WR_DATA_W),
      .USE_PACKETS      (0),
      .USE_EMPTY        (0),
      .PIPELINE_READY   (0),
      .SYNC_RESET       (SYNC_RESET),
    ...
    assign p1_rdatachk = ruser_datachk;
    assign p1_rpoison  = ruser_poison;
    assign p1_ruser_data   = ruser_data;
...
generate
   ...
   if(ENABLE_OOO ==1 && REORDER_BUFFER == 0)begin
       ...
       top_altera_merlin_axi_slave_ni_altera_avalon_st_pipeline_stage_19129_6ngu5fy #(
             .SYMBOLS_PER_BEAT (1),
...
ERROR_pkt_trans_write_not_asserted_when_write_cp_valid_is_asserted:
    assert property ( @(posedge aclk)
        disable iff (areset) (wr_cp_valid |-> wr_cp_data[PKT_TRANS_WRITE])
    );
...
// what do we do here in 4k splitter............................
// =====================**********************************================================
// ===============> CMD PATH
// check if we are crossing 4K boundary .. how do we do that ?
// start address + (AxLEN+1)*Size > 4K 
// will split the commands if crosses 4K, pass through otherwise
// new command address will be > {MSBs,LSBs -0s}
// some optimization ... this will happen only for Avalon Master - full size
// instead of runtime AxSize use compile time data width
// start address + (AxLEN+1)* DATA_W/8 > 4K 
// =====================**********************************================================
// ============***********************************************
// How is it used :- 
// What we do is write ORIGINAL data packet in FIFO so adapters dont worry in
// response path
// the generated source_cp_xxx is only used for command channels only
// About Read Response path ==>
// For read, we just need to ignore first rlast of 4k split CMD- 
// But we dont use rlast anyways, so we are good with read rsp path
//
// About Write Response path ==>
// we need to ignore first (or second) bvalid for WR RSP path
// we can add flag in CMD to determine split and use it for masking bvalid
// ============***********************************************

module burst_split_4k # (
    parameter  PKT_DATA_W      = 256,
    parameter  ST_DATA_W       = 256,
    parameter  ST_CHANNEL_W    = 12,
    parameter  PKT_ADDR_H      = 88,
    parameter  PKT_ADDR_L      = 84,
    parameter  PKT_BURST_SIZE_H  = 86,
    parameter  PKT_BURST_SIZE_L  = 84,
    parameter PADDING_ADDR_MERLIN = 83,
    parameter  PKT_BYTE_CNT_H  = 88,
    parameter  PKT_BYTE_CNT_L  = 84,
    parameter  ADDR_WIDTH      = 14,
    // derived    
    parameter  ST_DATA_OUT_W   = ST_DATA_W + (PKT_BYTE_CNT_H-PKT_BYTE_CNT_L)+1 // 1 bit indicating 4k split    
) (
    input aclk,
    input aresetn,
    // Av-st read sink command packet interface
    output                                    sink_cp_ready,
    input                                     sink_cp_valid,
    input             [ST_DATA_W-1:0]         sink_cp_data,
    input             [ST_CHANNEL_W-1:0]      sink_cp_channel,
    input                                     sink_cp_startofpacket,
    input                                     sink_cp_endofpacket,
    // Av-st write source command packet interface
    input  reg                                source_cp_ready,
    output                                    source_cp_valid,
    output reg        [ST_DATA_OUT_W-1:0]     source_cp_data, // additional bit to tell split cmd, first part
    output            [ST_CHANNEL_W-1:0]      source_cp_channel,
    output                                    source_cp_startofpacket,
    output reg                                source_cp_endofpacket
);
    // --------------------------------------------------
    // Ceil(log2()) function
    // --------------------------------------------------
...
localparam  BYTECOUNT_W         = PKT_BYTE_CNT_H - PKT_BYTE_CNT_L + 1;
localparam WORD_ALIGNED = log2ceil (PKT_DATA_W/8);
// wire declarations ...
logic crossing_4k,crossing_4k_valid,first_eop_4k;
logic [11:0] addr_lsb_12; // ignore MSB, only get address below 4K
logic [ADDR_WIDTH-1:0] next_addr_4k,next_addr_4k_reg;
logic [ADDR_WIDTH-1:0] source_cp_addr;
wire [BYTECOUNT_W-1:0]    sink_cmd_bytecount,sink_cmd_bytecount_1,sink_cmd_bytecount_1_reg,sink_cmd_bytecount_2,sink_cmd_bytecount_reg;
wire [ADDR_WIDTH-1:0]     sink_cp_addr,sink_cp_addr_reg;
reg  [ST_DATA_W-1:0]      sink_cp_data_reg;
logic [PKT_BURST_SIZE_H-PKT_BURST_SIZE_L:0] burst_size;
logic [PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1:0] log_size_ratio; 
// extract required pieces from data
assign burst_size = sink_cp_data[PKT_BURST_SIZE_H:PKT_BURST_SIZE_L];
assign log_size_ratio = WORD_ALIGNED - burst_size;
assign sink_cmd_bytecount = log_size_ratio[PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1] ? sink_cp_data[PKT_BYTE_CNT_H :PKT_BYTE_CNT_L] : sink_cp_data[PKT_BYTE_CNT_H :PKT_BYTE_CNT_L] >> log_size_ratio; 


generate
  if (PKT_ADDR_H > PKT_ADDR_L + ADDR_WIDTH -1) begin
       assign sink_cp_addr     = sink_cp_data[PKT_ADDR_L + ADDR_WIDTH -1 : PKT_ADDR_L];
       assign sink_cp_addr_reg = sink_cp_data_reg[PKT_ADDR_L + ADDR_WIDTH -1 : PKT_ADDR_L];
  end
  else if (PKT_ADDR_H < PKT_ADDR_L + ADDR_WIDTH -1) begin
       assign sink_cp_addr     = {{PADDING_ADDR_MERLIN{1'b0}},sink_cp_data[PKT_ADDR_H : PKT_ADDR_L]};
       assign sink_cp_addr_reg = {{PADDING_ADDR_MERLIN{1'b0}},sink_cp_data_reg[PKT_ADDR_H : PKT_ADDR_L]};
  end
  else begin 
       assign sink_cp_addr     = sink_cp_data[PKT_ADDR_H : PKT_ADDR_L];
       assign sink_cp_addr_reg = sink_cp_data_reg[PKT_ADDR_H : PKT_ADDR_L];
  end
endgenerate
// calc axlen for 4k split
// first byte-count is 4096-current addr 
// second byte-cnt is total bytecount - first byte-cnt
// thing to note is : use word aligned address while calculating byte_cnt
assign sink_cmd_bytecount_1 = crossing_4k ? (4096-sink_cp_addr[11:0]) : sink_cmd_bytecount;
assign sink_cmd_bytecount_1_reg = (4096-sink_cp_addr_reg[11:0]);
assign sink_cmd_bytecount_2 = sink_cmd_bytecount_reg - sink_cmd_bytecount_1_reg;
// calc address when 4K split
generate 
if (ADDR_WIDTH>=12) begin : ADDR_W_12_PLUS
    assign addr_lsb_12 = sink_cp_addr[11:0];
    assign next_addr_4k = crossing_4k ? {(sink_cp_addr[ADDR_WIDTH-1:12]+1'b1),12'h0} : {sink_cp_addr[ADDR_WIDTH-1:12],12'h0};
end else begin : ADDR_W_11_LESS
    assign addr_lsb_12 = sink_cp_addr;
    assign next_addr_4k = {ADDR_WIDTH{1'b0}};
end
endgenerate
assign crossing_4k = ((addr_lsb_12+ sink_cmd_bytecount )  > 4096 )? 1'b1 : 1'b0;

// generate internal valid for second command
always @ (posedge aclk)
    if (!aresetn)
        crossing_4k_valid <= 1'b0;
    else if (crossing_4k && sink_cp_valid && sink_cp_ready)
        crossing_4k_valid <= 1'b1;
    else if (crossing_4k_valid && source_cp_valid && source_cp_ready)
        crossing_4k_valid <= 1'b0;

// sink outputs assigned here
assign sink_cp_ready   = source_cp_ready && (!crossing_4k_valid);
// source outputs assigned here
assign source_cp_valid = sink_cp_valid || crossing_4k_valid;
assign source_cp_addr  = crossing_4k_valid ? next_addr_4k_reg : sink_cp_addr;
assign source_cp_channel = sink_cp_channel ; // unused in slave_ni - dont care 
assign source_cp_startofpacket = sink_cp_startofpacket ; // unused in slave_ni - dont care 
// latch data and reuse for post split cmd
always @ (posedge aclk)
    if (!crossing_4k_valid && sink_cp_valid)
        sink_cp_data_reg <= sink_cp_data;
assign sink_cmd_bytecount_reg =  log_size_ratio[PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1] ? sink_cp_data_reg[PKT_BYTE_CNT_H:PKT_BYTE_CNT_L] : sink_cp_data_reg[PKT_BYTE_CNT_H:PKT_BYTE_CNT_L] >> log_size_ratio;  

always @ (posedge aclk)
    if (sink_cp_valid & !crossing_4k_valid)
        next_addr_4k_reg <= next_addr_4k;

// insert new Len and ByteCount     
always_comb begin
    //default
    source_cp_data [ST_DATA_OUT_W-1] = crossing_4k; // indicates 4k crossover
    source_cp_data [ST_DATA_OUT_W-2:ST_DATA_OUT_W-2-(BYTECOUNT_W)] = sink_cmd_bytecount;
    source_cp_data[ST_DATA_W-1:0] = sink_cp_data;
    // for crossingg 4k_valid first override entire data
    if (crossing_4k_valid)
        source_cp_data[ST_DATA_W-1:0] = sink_cp_data_reg;
    // further specific overrides
    if (crossing_4k_valid)
         source_cp_data[PKT_BYTE_CNT_H :PKT_BYTE_CNT_L] = log_size_ratio[PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1] ? sink_cmd_bytecount_2 : (sink_cmd_bytecount_2 << log_size_ratio) ;    
    else
        source_cp_data[PKT_BYTE_CNT_H :PKT_BYTE_CNT_L] = log_size_ratio[PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1] ? sink_cmd_bytecount_1 : (sink_cmd_bytecount_1 << log_size_ratio);  

    source_cp_data[PKT_ADDR_H:PKT_ADDR_L] = source_cp_addr;
end
// latch incoming SOP for 4K split
always @ (aclk) 
    if (crossing_4k && sink_cp_valid ) 
        first_eop_4k <= sink_cp_endofpacket;

//supress EOP for first transaction of 4k split
always  @ (*)
    if (crossing_4k && sink_cp_valid) begin
        source_cp_endofpacket = 1'b0;
    end
    else if (crossing_4k_valid) begin
        source_cp_endofpacket = first_eop_4k;
    end
    else begin
        source_cp_endofpacket = sink_cp_endofpacket;
    end

endmodule

module wr_burst_split_4k # (
    parameter  PKT_DATA_W      = 256,
    parameter  ST_DATA_W       = 256,
    parameter  ST_CHANNEL_W    = 12,
    parameter  PKT_ADDR_H      = 88,
    ...
    parameter  ADDR_WIDTH      = 14,
    // derived    
    parameter  ST_DATA_OUT_W   = ST_DATA_W + (PKT_BYTE_CNT_H-PKT_BYTE_CNT_L+1)+1 // 1 bit indicating 4k split    
    // and block second
) (
    input aclk,
    input aresetn,
    ...
    input  reg                                source_cp_ready,
    output                                    source_cp_valid,
    output reg        [ST_DATA_W  :0]         source_cp_data, // additional bit to tell split cmd, first part
    output            [ST_CHANNEL_W-1:0]      source_cp_channel,
    output                                    source_cp_startofpacket,
    output reg                                source_cp_endofpacket
);
...
wire [BYTECOUNT_W-1:0]    minimum_bytecount;
reg  [BYTECOUNT_W-1:0]    updated_byte_cnt;
logic [PKT_BURST_SIZE_H-PKT_BURST_SIZE_L:0] burst_size;
logic [PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1:0] log_size_ratio; 
wire [31:0] minimum_bytecount_wire = PKT_DATA_W / 8; // to solve qis warning
assign minimum_bytecount = log_size_ratio[PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1] ? minimum_bytecount_wire : (1'b1 << burst_size);
// extract required pieces from data
...
assign sink_cmd_bytecount_1_reg = (4096-sink_cp_addr_reg[11:0]);
assign sink_cmd_bytecount_2 = sink_cmd_bytecount_reg - sink_cmd_bytecount_1_reg;
// first  (comes with sop) command give sink_cmd_bytecount_1 and then count down
// once reaches 0, reload to sink_cmd_bytecount_1_reg (rest of byte count)
always @ (posedge aclk)
    if (source_cp_valid && source_cp_ready && source_cp_startofpacket)
        updated_byte_cnt <= sink_cmd_bytecount_1 - minimum_bytecount;
    else if (source_cp_valid && source_cp_ready && (updated_byte_cnt==minimum_bytecount))
        updated_byte_cnt <= sink_cmd_bytecount_2;
    else if (source_cp_valid && source_cp_ready )
        updated_byte_cnt <= updated_byte_cnt- minimum_bytecount ;
// calc address when 4K split
generate 
if (ADDR_WIDTH>=12) begin : ADDR_W_12_PLUS
    assign addr_lsb_12 = sink_cp_addr[11:0];
    assign next_addr_4k = crossing_4k ? {(sink_cp_addr[ADDR_WIDTH-1:12]+1'b1),12'h0} : {sink_cp_addr[ADDR_WIDTH-1:12],12'h0};
end else begin : ADDR_W_12_LESS
    assign addr_lsb_12 = sink_cp_addr;
    assign next_addr_4k = {ADDR_WIDTH{1'b0}};
end
endgenerate
...
always @ (posedge aclk)
    if (!aresetn)
        crossing_4k_valid <= 1'b0;
    else if (source_cp_valid && source_cp_endofpacket)
        crossing_4k_valid <= 1'b0;
    else if (crossing_4k && sink_cp_valid && sink_cp_ready)
        crossing_4k_valid <= 1'b1;
    else if (crossing_4k_valid && source_cp_valid && source_cp_ready)
...
// sink outputs assigned here
assign sink_cp_ready   = source_cp_ready ;
// source outputs assigned here
assign source_cp_valid = sink_cp_valid;
assign source_cp_addr  = crossing_4k_valid ? next_addr_4k_reg : sink_cp_addr;
assign source_cp_channel = sink_cp_channel ; // unused in slave_ni - dont care 
assign source_cp_startofpacket = sink_cp_startofpacket ; // unused in slave_ni - dont care 
// latch data and reuse for post split cmd
...
always @ (posedge aclk)
    if (sink_cp_valid && sink_cp_startofpacket)
        next_addr_4k_reg <= next_addr_4k;

// insert new Len and ByteCount     
always_comb begin
    //default
    source_cp_data [ST_DATA_W] = crossing_4k; // indicates 4k crossover
    source_cp_data[ST_DATA_W-1:0] = sink_cp_data;
    // SOP byte-cnt is combinational, hence pass that through
    // after that updated counter will take care of it
    if (crossing_4k && source_cp_valid && source_cp_startofpacket) // load new  byte-counter first command
        source_cp_data[PKT_BYTE_CNT_H :PKT_BYTE_CNT_L] = log_size_ratio[PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1] ?  sink_cmd_bytecount_1 : (sink_cmd_bytecount_1 << log_size_ratio)   ;    
    else if (crossing_4k) // follow up counts
        source_cp_data[PKT_BYTE_CNT_H :PKT_BYTE_CNT_L] = log_size_ratio[PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1] ? updated_byte_cnt : (updated_byte_cnt << log_size_ratio);    
    else
        source_cp_data[PKT_BYTE_CNT_H :PKT_BYTE_CNT_L] = log_size_ratio[PKT_BURST_SIZE_H-PKT_BURST_SIZE_L+1] ? sink_cmd_bytecount : (sink_cmd_bytecount << log_size_ratio );    

    source_cp_data[PKT_ADDR_H:PKT_ADDR_L] = source_cp_addr;
end
assign source_cp_endofpacket = sink_cp_endofpacket;
// since we are splitting commands, we receive 2 responses
// we need to ignore one of it ....

always @ (posedge aclk)
    if (!aresetn)
        ignore_bvalid <= 0;
    else if (source_cp_startofpacket && source_cp_valid && crossing_4k)
        ignore_bvalid <= !ignore_bvalid;
endmodule
