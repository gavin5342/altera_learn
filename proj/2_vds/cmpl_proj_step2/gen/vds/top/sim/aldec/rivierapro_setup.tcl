...
# ACDS 26.1 110 linux 2026.04.22.02:07:53
# ----------------------------------------
...
# ----------------------------------------
# If top is one of several IP cores in your
...
append ELAB_OPTIONS [subst [top::get_elab_options $SIMULATOR_TOOL_BITNESS]]
append SIM_OPTIONS [subst [top::get_sim_options $SIMULATOR_TOOL_BITNESS]]
...
alias file_copy {
  ...
  set memory_files [concat $memory_files [top::get_memory_files "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
  ...
  set normalized_src [top::normalize_path "$file"]
  set normalized_dest [top::normalize_path "$dest_file"]
...
set libraries [dict merge $libraries [top::get_dpi_libraries "$QSYS_SIMDIR"]]
...
set design_libraries [dict merge $design_libraries [top::get_design_libraries]]
...
alias com {
  ...
  set design_files [dict merge [top::get_common_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR"]]
  set common_design_files [dict values $design_files]
  foreach file $common_design_files {
    eval $file
  }
  set design_files [list]
  set design_files [concat $design_files [top::get_design_files $USER_DEFINED_COMPILE_OPTIONS $USER_DEFINED_VERILOG_COMPILE_OPTIONS $USER_DEFINED_VHDL_COMPILE_OPTIONS "$QSYS_SIMDIR" "$QUARTUS_INSTALL_DIR"]]
