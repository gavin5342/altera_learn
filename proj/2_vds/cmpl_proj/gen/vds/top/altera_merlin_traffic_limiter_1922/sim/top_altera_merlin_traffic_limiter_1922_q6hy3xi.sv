...
module top_altera_merlin_traffic_limiter_1922_q6hy3xi
#(
   parameter
      PKT_TRANS_POSTED           = 1,
      PKT_DEST_ID_H              = 0,
   ...
   if (SYNC_RESET == 0) begin : async_reg1
      always @(posedge clk, posedge reset) begin
         if (reset) begin
            ...
            if ((count_is_0===1) && (response_accepted===1)) //making sure we are not showing error msg when count_is_0 and response_accepted are other than 1. 
     ...
     if (REORDER) begin: fifo_dest_id_write_read_control_reorder_on
       ...
       if (ENABLE_OOO == 0 | USE_FIFO == 0) begin : REORDER_WITHOUT_FIFO_OR_INORDER_RSP
         ...
         if (ENABLE_OOO == 0) begin : REORDER_for_in_order_rsp
           ...
           top_altera_merlin_traffic_limiter_altera_avalon_sc_fifo_1922_xaundgi #(
                 .SYMBOLS_PER_BEAT   (1),
