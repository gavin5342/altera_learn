...
module top_altera_merlin_axi_master_ni_19117_a33l7ua
#( 
    //----------------
   ...
   parameter  ID_WIDTH = 2,
    ...
    //-------------------------
            ...
            PKT_AWATOP_L        = 154,
            PKT_AWATOP_H        = 159,
            PKT_AWSTASHNID_L    = 160,
            PKT_AWSTASHNID_H    = 170,
            PKT_AWSTASHNIDEN    = 171,
            PKT_AWSTASHLPID_L   = 172,
            PKT_AWSTASHLPID_H   = 176,
            PKT_AWSTASHLPIDEN   = 177,
            PKT_MMUSECSID       = 178,
            PKT_MMUSID_L        = 179,
            PKT_MMUSID_H        = 195,
    ...
    //----------------
            ...
            DATACHK_WIDTH     = PKT_DATA_W / 8,
            POISON_WIDTH      = (PKT_DATA_W + 64 -1) / 64 , // ceil(WDATA_WIDTH/64) workaround 
...
(
    ...
    input [DATACHK_WIDTH-1:0]            wdatachk,
    input [POISON_WIDTH-1:0]             wpoison,
    input                                wtrace,    
    output                               wready,
    //----------------
    ...
    output reg                           btrace,
    input                                bready,
    output reg [RESP_USER_WIDTH-1:0]     buser, 
    //----------------
    ...
    input [1:0]                          awdomain, 
    input [AWSNOOP_WIDTH-1:0]            awsnoop,  
    input [1:0]                          awbar,    
    input                                awunique,
    ...
    output                               rlast,
    output                               rvalid,
    output reg [DATACHK_WIDTH-1:0]       rdatachk,
    output reg [POISON_WIDTH-1:0]        rpoison,
    output reg                           rtrace,     
    input                                rready,
    ...
    // ACE5Lite
    input [5:0]                          awatop,
    input [10:0]                         awstashnid,  
    input                                awstashniden,      
    input [4:0]                          awstashlpid, 
    input                                awstashlpiden, 
    input                                awmmusecsid,
    input [SID_WIDTH-1:0]                awmmusid,
    input                                armmusecsid,
    input [SID_WIDTH-1:0]                armmusid,
    input [ADDRCHK_WIDTH-1:0]            awaddrchk,
    input [ADDRCHK_WIDTH-1:0]            araddrchk,
    //----------------------------
...
reg                                  awvalid_q0;
reg [1:0]                           awdomain_q0; 
reg [AWSNOOP_WIDTH-1:0]              awsnoop_q0;  
reg [1:0]                              awbar_q0;   
reg                                 awunique_q0;
...
reg                                   wtrace_q0;
reg [POISON_WIDTH-1:0]              wpoison_q0;
reg [DATACHK_WIDTH-1:0]            wdatachk_q0; 
...
reg [5:0]                             awatop_q0;
reg [10:0]                        awstashnid_q0;  
reg                             awstashniden_q0;      
reg [4:0]                        awstashlpid_q0; 
reg                            awstashlpiden_q0; 
reg                              awmmusecsid_q0;
reg [SID_WIDTH-1:0]                 awmmusid_q0;
reg                              armmusecsid_q0;
reg [SID_WIDTH-1:0]                 armmusid_q0;
reg [ADDRCHK_WIDTH-1:0]            awaddrchk_q0;
reg [ADDRCHK_WIDTH-1:0]            araddrchk_q0;
...
reg data_less_txn, data_less_txn_q0;
generate 
       if (AXI_VERSION == "AXI5" && ACE5_LITE_SUPPORT == 1) begin
           assign data_less_txn = ( awvalid && awready && (awsnoop == 'b1100 || awsnoop == 'b1101) && (awbar[0] == 0) && (awlock == 0) && (awdomain == 'b00 || awdomain == 'b01 || awdomain == 'b10) && (awcache[1] == 'b1) ) ;
       end else begin
            assign data_less_txn = '0 ;
       end
endgenerate
...
always @ (posedge aclk) begin
   if(~awvalid_q0 || (wvalid_q0 & wlast_q0 & network_ready)) begin 
      ...
      awatop_q0        <= awatop;
      awstashnid_q0    <= awstashnid;
      awstashniden_q0  <= awstashniden;
      awstashlpid_q0   <= awstashlpid;
      awstashlpiden_q0 <= awstashlpiden;
      awmmusecsid_q0   <= awmmusecsid;
      awmmusid_q0      <= awmmusid;
      ...
      data_less_txn_q0 <= data_less_txn;
...
always @ (posedge aclk) begin
   if((network_ready && awvalid_q0) || ~wvalid_q0) begin
      ...
      wdata_q0  <=    wdata;
      wstrb_q0  <=    wstrb;
      wlast_q0  <=    (data_less_txn) ? 1'h1 : wlast;
      wuser_q0  <=    wuser;
      if (ROLE_BASED_USER) begin
...
always @ (posedge aclk) begin
   if(read_cp_ready || ~arvalid_q0) begin
      ...
      arsnoop_q0     <=  arsnoop;
      arbar_q0       <=    arbar;
      armmusecsid_q0 <= armmusecsid;
      armmusid_q0    <= armmusid;
      if (ROLE_BASED_USER) begin
...
reg [31 : 0]            burstwrap_value_read;
    ...
    generate
        if (AXI_VERSION != "AXI4Lite" && AXI_VERSION != "AXI5Lite"  ) begin
          //--------------------------------------
       ...
       if (!aresetn)
           ...
           begin
              ...
              if (data_less_txn)
                  ...
                  if (write_addr_data_both_valid && write_cp_ready) begin
                      sop_enable <= 1'b0;
                      if (wlast_q0)
                         sop_enable <= 1'b1;
                  end
...
assign write_cp_endofpacket         = (data_less_txn) ? 1'h1 : wlast_q0;   
...
always_comb  
    begin
        ...
        write_cp_data[PKT_BURSTWRAP_H:PKT_BURSTWRAP_L]      = (AXI_VERSION == "AXI4Lite" || AXI_VERSION == "AXI5Lite") ? 0 : burstwrap_value_write[PKT_BURSTWRAP_W - 1 : 0];
        ...
        write_cp_data[PKT_AWATOP_H:PKT_AWATOP_L]           = awatop_q0;
        write_cp_data[PKT_AWSTASHNID_H:PKT_AWSTASHNID_L]   = awstashnid_q0;
        write_cp_data[PKT_AWSTASHNIDEN]                    = awstashniden_q0;
        write_cp_data[PKT_AWSTASHLPID_H:PKT_AWSTASHLPID_L] = awstashlpid_q0;
        write_cp_data[PKT_AWSTASHLPIDEN]                   = awstashlpiden_q0;
        write_cp_data[PKT_MMUSECSID]                       = awmmusecsid_q0;
        write_cp_data[PKT_MMUSID_H:PKT_MMUSID_L]           = awmmusid_q0;
        ...
        if (ROLE_BASED_USER) begin
            ...
            if(USE_PKT_DATACHK == 0)
                    write_cp_data[PKT_DATACHK_H:PKT_DATACHK_L]          = calcParity(wdata_q0);            
            else
        ...
        if(ACE_LITE_SUPPORT == 1 || ACE5_LITE_SUPPORT == 1) begin
            write_cp_data[PKT_DOMAIN_H : PKT_DOMAIN_L] = awdomain_q0;
            if (ACE5_LITE_SUPPORT == 1) 
                write_cp_data[PKT_SNOOP_H : PKT_SNOOP_L]   = awsnoop_q0;  // For consistency between write and read packets
            else 
        ...
        end else if (AXI_VERSION == "AXI5LITE" || AXI_VERSION == "AXI4Lite" ) begin 
        ...
        write_cp_data[PKT_DATALESS] = data_less_txn_q0;
...
always_comb  
    begin
        ...
        read_cp_data[PKT_BURSTWRAP_H:PKT_BURSTWRAP_L]       = (AXI_VERSION == "AXI4Lite" || AXI_VERSION == "AXI5Lite") ? 0 : burstwrap_value_read[PKT_BURSTWRAP_W - 1 : 0];
        ...
        read_cp_data[PKT_MMUSID_H : PKT_MMUSID_L]   = armmusid_q0;
        read_cp_data[PKT_MMUSECSID]                 = armmusecsid_q0; 
 
        // AXI4 signals: receive from translator
        if (AXI_VERSION == "AXI4" || AXI_VERSION == "AXI5") begin
...
// synthesis translate_off
    generate
        if (WRITE_ISSUING_CAPABILITY > 0) begin
            ...
            assign write_response_count_is_1  = (pending_write_response_count >= WRITE_ISSUING_CAPABILITY);
            // assertion
