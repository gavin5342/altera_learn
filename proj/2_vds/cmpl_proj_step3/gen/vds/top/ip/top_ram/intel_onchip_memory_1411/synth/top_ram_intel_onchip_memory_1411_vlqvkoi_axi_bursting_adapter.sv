...
module  top_ram_intel_onchip_memory_1411_vlqvkoi_axi_bursting_adapter  #(
...
// Case:15014689058 with ECC enabled (AXI-4 master's rready will be asserted only when rvalid is asserted)
// Bug: w_rdaddr_done did not assert even when rden_count=0 because:
// Bug: when w_ram_rready is de-asserted; rready is asserted.
// Bug: when w_ram_rready is asserted;    rready is already de-asserted (because there is no rvalid, which is expected)
//assign w_rdaddr_done = ~(|rden_count) & rready & w_ram_rready;
assign w_rdaddr_done = ~(|rden_count) & (rready | pending_rdaddr_done) & w_ram_rready;


always @(posedge clk) begin
    if (reset)
        pending_rdaddr_done <= 1'b0;
    else if (w_rdaddr_done)
        pending_rdaddr_done <= 1'b0;
    // to re-assert w_rdaddr_done when w_ram_rready=1 now
    // when previous cycle is rden_count=0 and rready=1 but w_ram_rready=0(backpressure due to rmw)
    else if (~(|rden_count) && rready)
        pending_rdaddr_done <= 1'b1;
end
